            ...
            try
            {
                if (canGenerateExcel())
                {
                    ...
                    for (int i = 0; i < dgv.Rows.Count; i++)
                    {
                        ...
                        amount = double.TryParse(dgv.Rows[i].Cells["doctotal"].Value.ToString(), out doubleTemp) ? Convert.ToDouble(dgv.Rows[i].Cells["doctotal"].Value.ToString()) : doubleTemp;
        ...
        public async void loadData()
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    Console.WriteLine("/api/sales/get_all" + sDocStatus + sSAPNumber + sBranch + sWarehouse + sTransdate + sCustType + sSearch);
