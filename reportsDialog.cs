...
namespace AB
{
    ...
    {
        string gReportType = "", gTransDate = "", gBranch = "", gBranchName = "", gPrintedBy = "";
        public reportsDialog(string reportType,string transdate,string branch,string branchName,string printedBy)
        {
            gReportType = reportType;
            gTransDate = transdate;
            gBranch = branch;
            gBranchName = branchName;
            gPrintedBy = printedBy;
            InitializeComponent();
        }
          ...
          try
            {
                ...
                if (gReportType.Equals("Final Count Report"))
                {
                    ...
                    finalCount.SetParameterValue("date_report", gTransDate);
                    finalCount.SetParameterValue("branch", gBranchName);

                    if (!string.IsNullOrEmpty(gPrintedBy.Trim()))
                    {
                        finalCount.SetParameterValue("printed_by", "");
                    }
                ...
                {
                    finalCount_class finalCountc = new finalCount_class(gReportType,gTransDate,gBranch);
                    string result = finalCountc.loadRequest();
                    dtResult = loadFinal(result);
                    finalReport finalReport = new finalReport();
                    finalReport.Subreports[3].Database.Tables["sub_report1"].SetDataSource(loadFinal_SubQuery1(result));
                    finalReport.Subreports[4].Database.Tables["payment_method"].SetDataSource(loadPaymentMethod(result));
                    finalReport.Subreports[0].Database.Tables["discountTypeSummary"].SetDataSource(loadDiscountTypeSummary(result));
                    finalReport.Subreports[1].Database.Tables["employee_charge"].SetDataSource(loadEmployeeCharge(result));
                    ...
                    finalReport.SetParameterValue("selectedBranch", gBranchName);
                    finalReport.SetParameterValue("date_report", gTransDate);

                    if (!string.IsNullOrEmpty(gPrintedBy.Trim()))
                    {
                        finalReport.SetParameterValue("printed_by", "");
                    }
                ...
                else if (gReportType.Equals("Final Sales Report"))
                {
                    ...
                    finalReport.Subreports[0].Database.Tables["discountTypeSummary"].SetDataSource(loadDiscountTypeSummary(result));
                    finalReport.Subreports[1].Database.Tables["row"].SetDataSource(loadFinal_SubQuery1(result));
                    finalReport.Subreports[2].Database.Tables["payment_method"].SetDataSource(loadPaymentMethod(result));
                    finalReport.SetParameterValue("date_report",gTransDate);
                    finalReport.SetParameterValue("branch", gBranchName);
        ...
        public DataTable loadPaymentMethod(string responseContent)
        {
            DataTable dt = new DataTable();
            finalCount_class finalCountc = new finalCount_class(gReportType, gTransDate, gBranch);
            dt = finalCountc.loadPaymentMethod(EnterDate.dateEntered, gReportType, EnterDate.branchCode, responseContent);
            return dt;
        }

        public DataTable loadDiscountTypeSummary(string responseContent)
        {
            DataTable dt = new DataTable();
            finalCount_class finalCountc = new finalCount_class(gReportType, gTransDate, gBranch);
            dt = finalCountc.loadDiscountTypeSummary(EnterDate.dateEntered, gReportType, EnterDate.branchCode,responseContent);
            return dt;
        }

        public DataTable loadEmployeeCharge(string responseContent)
        {
            DataTable dt = new DataTable();
            finalCount_class finalCountc = new finalCount_class(gReportType, gTransDate, gBranch);
            dt = finalCountc.loadEmployeeCharge(EnterDate.dateEntered, gReportType, EnterDate.branchCode, responseContent);
            return dt;
        }

        public DataTable loadFinal_SubQuery1(string responseContent)
        {
            DataTable dt = new DataTable();
            //CASH
            ...
            dt.Columns.Add("total_cash_out", typeof(double));
        ...
        public DataTable loadFinal(string responseContent)
        {
            DataTable dt = new DataTable();
            //CASH
            dt.Columns.Add("total_cash_on_hand", typeof(double));
        ...
        public DataTable loadFinalCount()
        {
            ...
            DataTable dtFromAPI = new DataTable();
            finalCount_class finalCountc = new finalCount_class(gReportType, gTransDate, gBranch);
            dtFromAPI = finalCountc.loadFinalCount(Convert.ToDateTime(gTransDate), gBranch);
            foreach (DataRow row in dtFromAPI.Rows)
            {
