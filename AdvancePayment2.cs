...
namespace AB
{
    public partial class AdvancePayment2 : Form
    {
        ...
        private void AdvancePayment_Load(object sender, EventArgs e)
        {
            ...
            cmbStatus.SelectedIndex = gType.Equals("In Deposit") ? 1 : 0;
            ...
            if(dtResponse.Rows.Count > 0)
            {
                ...
                foreach (DataRow r0w in dtResponse.Rows)
                {
                    ...
                    if (!string.IsNullOrEmpty(txtSearch.Text.ToString().Trim()))
                    {
                        ...
                        {
                            dgv.Rows.Add(r0w["id"], Convert.ToDateTime(r0w["transdate"]).ToString("yyyy-MM-dd HH:mm"), r0w["reference"], r0w["cust_code"], Convert.ToDecimal(string.Format("{0:0.00}", amount)), Convert.ToDecimal(string.Format("{0:0.00}", balance)), r0w["remarks"], r0w["sap_number"], r0w["reference2"], r0w["status"]);
                        }
        ...
        private void dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(dgv.Rows.Count > 0)
            {
                ...
                {
                    int idd = String.IsNullOrEmpty(dgv.CurrentRow.Cells["id"].Value.ToString()) ? 0 : Convert.ToInt32(dgv.CurrentRow.Cells["id"].Value.ToString());
                    ItemDeposit itemDeposit = new ItemDeposit(idd);
