...
namespace AB.API_Class.Reports
{
    class finalCount_class
    {
        ...
        public finalCount_class(string type,string transDate, string branch)
        {
            reportType = type;
        ...
        public string reportType = "", sTransDate = "", sBranch = "";
        public string loadRequest()
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    string URL = reportType.Equals("Final Report") ? "/api/report/final_report" : "/api/sales/final_report";
                    URL += "?transdate=" + sTransDate + "&branch=" + sBranch;
        ...
        public DataTable loadFinalCount(DateTime dateEntered, string branchCode)
        {
            ...
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    request.AddHeader("Authorization", "Bearer " + token);
                    var response = client.Execute(request);
                    Console.WriteLine("/api/report/final_count?transdate=" + dateEntered.ToString("yyyy-MM-dd") + "&branch=" + branchCode);
        ...
        public DataTable loadPaymentMethod(DateTime dateEntered, string type, string branchCode, string responseContent)
        {
            DataTable dt = new DataTable();
            Cursor.Current = Cursors.WaitCursor;
            JObject jObject = new JObject();
                    ...
                    if (x.Key.Equals("data"))
                    {
                        if (x.Value.ToString() != "{}")
                        {
                            ...
                            foreach (var y in joData)
                            {
                                ...
                                {
                                    dt = (DataTable)JsonConvert.DeserializeObject(y.Value.ToString(), (typeof(DataTable)));
                                    break;
                                }
                            }
                        }
            ...
            if (dt.Rows.Count <= 0)
            {
                ...
                dt.Columns.Add("cash_sales");
                dt.Columns.Add("ar_sales");
                dt.Columns.Add("agent_sales");
                dt.Columns.Add("total");
            }
            return dt;
        }

        public DataTable loadEmployeeCharge(DateTime dateEntered, string type, string branchCode, string responseContent)
        {
            DataTable dt = new DataTable();
            Cursor.Current = Cursors.WaitCursor;
            JObject jObject = new JObject();
        ...
        public DataTable loadDiscountTypeSummary(DateTime dateEntered, string type, string branchCode, string responseContent)
        {
            DataTable dt = new DataTable();
            utility_class utilityc = new utility_class();
            Cursor.Current = Cursors.WaitCursor;
        ...
        public DataTable loadFinal(DateTime dateEntered, string type, string branchCode, string responseContent)
        {
            utility_class utilityc = new utility_class();
            DataTable dt = new DataTable();
            //CASH
                ...
                foreach (var x in jObject)
                {
                    if (x.Key.Equals("data"))
                    {
                        if (x.Value.ToString() != "{}")
                        {
                            ...
                            joData = JObject.Parse(x.Value.ToString());
                            //CASH
                            double totalCashOnHand = 0.00, totalCashPayment = 0.00, totalCashDeposit = 0.00, totalUsedDepositPayment = 0.00, totalBankDepositPayment = 0.00, totalEpayPayment = 0.00, totalGcPayment = 0.00, totalCashOut = 0.00;
                            //SALES
                                    ...
                                    if (y.Value.ToString() != "[]")
                                    {
                                        ...
                                        for (int i = 0; i < jsonArray.Count(); i++)
                                        {
                                            ...
                                            foreach (var q in data)
                                            {
                                                ...
                                                else if (q.Key.Equals("total_cash_out"))
                                                {
                                                    totalCashOut = (q.Value.ToString() == "" ? 0.00 : Convert.ToDouble(q.Value.ToString()));
                                                }
                                            }
                                        }
                                    }
