        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                string transdate = dgv.Rows[i].Cells["transdate"].Value.ToString(),
                    ...
                    sales = dgv.Rows[i].Cells["amt_in"].Value == null ? "" : Convert.ToDouble( dgv.Rows[i].Cells["amt_in"].Value.ToString()).ToString("n2"),
                    payment = dgv.Rows[i].Cells["amt_out"].Value == null ? "" : Convert.ToDouble(dgv.Rows[i].Cells["amt_out"].Value.ToString()).ToString("n2"),
                    runningBalance = dgv.Rows[i].Cells["running_balance"].Value == null ? "" : Convert.ToDouble(dgv.Rows[i].Cells["running_balance"].Value.ToString()).ToString("n2"),
                    remarks = dgv.Rows[i].Cells["remarks"].Value.ToString(),
                    arNumber = dgv.Rows[i].Cells["ar_num"].Value.ToString(),
                    IpNumber = dgv.Rows[i].Cells["ip_num"].Value.ToString(),
            ...
            if(dgv.Rows.Count > 0)
            {
                if(e.ColumnIndex==2 && e.RowIndex >= 0)
                {
                    ...
                    else
                    {
                        ...
                        frm.remarks = dgv.CurrentRow.Cells["remarks"].Value.ToString();
                        frm.remarks = dgv.CurrentRow.Cells["remarks"].Value.ToString();
                        frm.dtTransDate = Convert.ToDateTime(dgv.CurrentRow.Cells["transdate"].Value.ToString());
                                            ...
                                            foreach (var q in data)
                                            {
                                                ...
                                                else if (q.Key.Equals("row_result"))
                                                {
                                                    ...
                                                    for (int i = 0; i < jsonArray.Count(); i++)
                                                    {
                                                        ...
                                                        dgv.Rows.Add(id, dtTransDate.ToString("yyyy-MM-dd"), ref1, ref2, transType, amtIn <=0 ? (double?) null : amtIn, amtOut <=0 ?  (double?)null : amtOut, Convert.ToDecimal(string.Format("{0:0.00}", totalRunningBalance)), remarks, arNum, ipNum);
                                                    }
                                                }
                                            }
                                        }
