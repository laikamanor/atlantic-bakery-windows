...
namespace AB
{
    public partial class GoodsIssued_ForIssue : Form
    {
        public GoodsIssued_ForIssue()
        {
            InitializeComponent();
        }
        api_class apic = new api_class();
        devexpress_class devc = new devexpress_class();
        private void GoodsIssued_ForIssue_Load(object sender, EventArgs e)
        {
                ...
                if (IsHandleCreated)
                {
                    gridControl1.Invoke(new Action(delegate ()
                    {
                        ...
                        foreach (GridColumn col in gridView1.Columns)
                        {
                            ...
                            col.Visible = !(fieldName.Equals("id") || fieldName.Equals("issued") || fieldName.Equals("production_status") || fieldName.Equals("docstatus"));
        ...
        private void repositoryItemTextEdit1_Click(object sender, EventArgs e)
        {
            ...
            {
                GoodsIssued_ForIssue_Details.isSubmit = false;
                GoodsIssued_ForIssue_Details frm = new GoodsIssued_ForIssue_Details(id,selectedReference);
                frm.ShowDialog();
                if(GoodsIssued_ForIssue_Details.isSubmit)
                {
                    bg();
                }
            }
